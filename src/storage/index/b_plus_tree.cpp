...
BPLUSTREE_TYPE::BPlusTree(std::string name, page_id_t header_page_id, BufferPoolManager *buffer_pool_manager,
  ...
  std::string loginfo = ":Initialize tree with leaf size: " + std::to_string(leaf_max_size) +
...
auto BPLUSTREE_TYPE::GetValue(const KeyType &key, std::vector<ValueType> *result, Transaction *txn) -> bool {
  std::string loginfo = "Search for key: " + std::to_string(key.ToString());
  LOG_DEBUG("%s", loginfo.c_str());
...
auto BPLUSTREE_TYPE::Insert(const KeyType &key, const ValueType &value, Transaction *txn) -> bool {
  std::string loginfo = "Insert key: " + std::to_string(key.ToString());
  LOG_DEBUG("%s", loginfo.c_str());
  ...
  if (IsEmpty()) {
    ...
    loginfo = "New leaf page with id " + std::to_string(root_page_id);
    LOG_DEBUG("%s", loginfo.c_str());
  ...
  if (leaf_page->GetSize() == leaf_page->GetMaxSize()) {
    ...
    loginfo = "New leaf page with id " + std::to_string(new_page_id);
    LOG_DEBUG("%s", loginfo.c_str());
  ...
  while (!ctx.write_set_.empty()) {
    ...
    if (inner_page->GetSize() == inner_page->GetMaxSize()) {
      ...
      int split_id = (max_size + 1) / 2;
      bool insert_to_lower = false;
      bool insert_is_lift = true;

      int right_first_insert_id = split_id;
      KeyType lift_key = next_insert_key;
      ...
      loginfo = "New internal page with id " + std::to_string(new_page_id);
      LOG_DEBUG("%s", loginfo.c_str());

      new_inner_page->Init(internal_max_size_);
      new_inner_page->SetKeyAt(0, lift_key);
...
void BPLUSTREE_TYPE::Remove(const KeyType &key, Transaction *txn) {
  std::string loginfo = "Remove key: " + std::to_string(key.ToString());
  LOG_DEBUG("%s", loginfo.c_str());
  if (IsEmpty()) {
    return;
  }
...
auto BPLUSTREE_TYPE::Begin() -> INDEXITERATOR_TYPE {
  std::string loginfo = "Initiate iterator begin()";
  LOG_DEBUG("%s", loginfo.c_str());
...
auto BPLUSTREE_TYPE::Begin(const KeyType &key) -> INDEXITERATOR_TYPE {
  std::string loginfo = "Initiate iterator with key: " + std::to_string(key.ToString());
  LOG_DEBUG("%s", loginfo.c_str());
