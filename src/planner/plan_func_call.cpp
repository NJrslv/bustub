...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if(args.size() != 1) {
    throw Exception(fmt::format("wrong amount of arguments {}, should be 1", args.size()));
  }

  return func_name == "upper" ? std::make_shared<StringExpression>(args[0], StringExpressionType::Upper)
                              : std::make_shared<StringExpression>(args[0], StringExpressionType::Lower);
}
